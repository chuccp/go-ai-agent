...
在 Go AI Agent 中，**應用就是流程**。沒有獨立的「套件」或「技能」管理 — 你建立一個應用，透過聊天或視覺化方式設計其流程，然後執行它。
...
1. **聊天即開發** — 不需要寫程式碼，用自然語言描述需求，AI 智慧體自動設計、建構並執行工作流程。從想法到應用，只需一次對話。

2. **開箱即用** — 下載即用，無需配置資料庫、無需部署伺服器。桌面模式首次執行自動完成所有初始化，只需填入 API 金鑰即可開始使用。

3. **本地執行，資料在手** — 應用和資料全部儲存在本地裝置，不依賴雲端服務。對話記錄、流程配置、模型金鑰都在你自己的電腦上。

4. **靈活的流程編排** — 視覺化拖拽設計複雜工作流程，支援條件分支、迴圈、批次處理、平行執行。17 種節點類型滿足從簡單問答到複雜自動化的各種場景。

5. **多模型自由切換** — 支援 OpenAI、Claude、Gemini、DeepSeek 等 28+ 模型供應商，每個節點可以獨立選擇模型，靈活組合不同 AI 的能力。

6. **多模態 AI 能力** — 不僅是文字對話，還支援圖像生成、音訊生成、影片生成，將多種 AI 能力串聯在一個流程中。

7. **一鍵分享與復用** — 應用可匯出為 ZIP 套件，一鍵匯入到其他實例。社群可以共用工作流程範本，開箱即用。

8. **跨平台桌面體驗** — Windows、macOS、Linux 原生桌面應用，啟動快、體積小、資源佔用低，同時支援 Web 瀏覽器存取。

## 與其他產品有什麼不同？

### 與 ChatGPT、Claude 等 AI 對話工具

ChatGPT、Claude 這類產品是非常強大的通用 AI 工具。

但純對話有一個天然問題：**對話越長，上下文越臃腫，模型的注意力越分散**。當你在一個長對話中嘗試完成多個步驟時，前面的內容會干擾後面的判斷，輸出品質隨對話長度下降。

Go AI Agent 透過**流程節點**解決了這個問題：

- 每個節點只接收上游節點的輸出作為上下文，而不是整個對話歷史
- 每次 LLM 調用都是**聚焦的、乾淨的**，不受無關資訊干擾
- 不同節點可以選擇不同模型，各取所長

這意味著同樣的任務，流程化執行的每個步驟都比在長對話中執行**更專注、更準確**。

此外，建立好的流程可以匯出分享，別人匯入即可執行，不需要重複搭建。
